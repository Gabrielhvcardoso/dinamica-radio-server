...
export default async function indexWithCategories (req, res) {
  if (!req.body?.clientId) return res.sendStatus(404)

  const { clientId } = req.body

  const response = await knex('program')
  ...
  const reduced = Object.values(response.reduce((acc, item) => {
    const project = {
